...
@section('content')
    <div class="content login-container">
        ...
        <div class="col-md-4 ml-auto mr-auto login-form">
            <form role="form" method="POST" action="{{ route('login') }}">
                @csrf
            <div class="card card-login card-plain">
                <div class="card-header ">
                <div class="logo-container">
                    <img src="{{ asset('assets/img/uees-blanco.png') }}" alt="">
                </div>
                </div>
                <div class="card-body ">
                ...
                <button  type = "submit" class="btn bg-light btn-round btn-lg btn-block mb-3">{{ __('Ingresar') }}</button>
                <div >
                    <h6 class="wrapper-link-footer">
                    {{--<a href="{{ route('password.request') }}" class="link footer-link">{{ __('¿Recuperar Contraseña?') }}</a>--}}
                    <a href="#" class="link footer-link">{{ __('¿Recuperar Contraseña?') }}</a>
                    </h6>                
                </div>
                </div>
            </div>
